...
THIS IS THE FINAL VERSION, COMBINED WITH GUI COMMUNICATION.
*/
...
public:
  Brain() : Node("brain_node"), game_state_(PLAYER_TURN) {
    ...
    // --- Original Clients for Arm/Gripper ---
    ...
    // --- NEW: Subscribers/Services (GUI -> C++ Node) ---
    ...
    RCLCPP_INFO(this->get_logger(), "Brain node started. Waiting for services and GUI...");
    ...
    // Publish initial counts on startup
    publish_all_metrics();
    status_pub_->publish(std_msgs::msg::String().set__data("Ready for a new game?"));
  }
...
private:
  ...
  // MODIFIED: This function now checks for game over
  void onHumanMove(int cell) {
    if (game_play_->makeMove(cell, TicTacToe::BLACK)) {
      
      // --- NEW: Check if human won ---
      if (game_play_->isGameOver()) {
        handle_game_over(TicTacToe::BLACK);
        return;
      }

      game_state_ = ROBOT_TURN;
      status_pub_->publish(std_msgs::msg::String().set__data("Robot is thinking..."));


      int computerMove = game_play_->findBestMove();
      RCLCPP_INFO(get_logger(), "Computer selected move %d", computerMove);

      moveRobotToCell(computerMove); // This starts the robot motion
      game_play_->makeMove(computerMove, TicTacToe::WHITE);

      // --- NEW: Check if robot won ---
      if (game_play_->isGameOver()) {
        // The arm response will handle the state change
        // We just need to know the game is over
    ...
    switch (current_action_) {
        ...
        case MOVE_TO_PLACE:
            ...
            if (using_gripper) {
                ...
                // --- NEW: Check game state before setting to PLAYER_TURN ---
                if (game_state_ == FINISHED) {
                    handle_game_over(TicTacToe::WHITE);
                } else {
                    game_state_ = PLAYER_TURN;
                    status_pub_->publish(std_msgs::msg::String().set__data("It's your turn!"));
                }
            }
            break;
  ...
  // --- NEW: GUI Communication Functions ---
                      ...
                      std::shared_ptr<std_srvs::srv::Trigger::Response> response)
  {
      ...
      // 2. Call the (assumed) hardware/arm stop service
      if (arm_stop_client_ && arm_stop_client_->service_is_ready()) {
          ...
          // Send the request and wait for the arm to confirm the stop
          auto future = arm_stop_client_->async_send_request(stop_req);
          
          if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future, std::chrono::seconds(2)) ==
              rclcpp::FutureReturnCode::SUCCESS)
          {
              RCLCPP_INFO(get_logger(), "Arm stop service confirmed.");
              response->success = true;
              response->message = "E-Stop confirmed by brain and arm.";
          } else {
              RCLCPP_ERROR(get_logger(), "Arm stop service failed to respond!");
              response->success = false;
              response->message = "E-Stop triggered, but arm failed to confirm stop!";
          }
      } else {
  ...
  // --- Original Subscriptions/Clients ---
  ...
  // --- NEW: GUI Communication Variables ---
